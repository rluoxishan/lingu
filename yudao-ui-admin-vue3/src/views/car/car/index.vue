    ...
    <div class="vehicle-page__card">
      ...
      <div class="vehicle-page__filters">
        <el-input
          ...
          placeholder="搜索设备ID / 名称 / 区域 / 描述 / 任务"
          clearable
        ...
        <el-select v-model="queryParams.online" class="vehicle-page__select" @change="handleQuery">
          <el-option
        ...
        <el-button link type="primary" @click="resetQuery">重置筛选</el-button>
...
import { mergeDetailIntoStatus } from '@/api/car/vehicle/mapper'
...
const list = ref<VehicleStatusVO[]>([])
...
const handleQuery = () => applyFilters(allList.value)
...
const handleDetail = async (row: VehicleStatusVO) => {
  ...
  if (useMock) return
  try {
    const detail = await VehicleApi.selectDeviceDetail(row.vehicleId)
    const next = { ...row }
    mergeDetailIntoStatus(next, detail)
...
const handleRegister = () => message.info('注册新车（待对接写接口）')
const handleBatchEmergencyStop = () => message.warning('批量急停（待对接写接口）')
const handleEdit = (row: VehicleStatusVO) => message.info(`编辑 ${row.vehicleId}（待对接）`)
...
const handleMonitor = (row: VehicleStatusVO) => {
  router.push(`/car/car/monitor/${row.vehicleId}`)
}
...
const handleDelete = async (row: VehicleStatusVO) => {
  ...
  try {
    await message.delConfirm(`确认删除设备「${row.vehicleId}」吗？删除后不可恢复。`)
    allList.value = allList.value.filter((item) => item.vehicleId !== row.vehicleId)
