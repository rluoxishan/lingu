<template>
  <div class="remote-drive" :class="`remote-drive--mode-${controlMode}`">
    <div class="remote-drive__topbar">
      <div class="remote-drive__status-group">
        ...
        <span
          ...
          :class="takeover ? 'remote-drive__pill--active' : 'remote-drive__pill--idle'"
        >
          {{ modeStatusText }}
        </span>
      </div>
      ...
      <div class="remote-drive__network">网络正常 · {{ formatTime() }} · {{ temperature }}°C</div>
    </div>
    ...
    <div v-if="controlMode === 'auto'" class="remote-drive__auto">
      ...
      <div class="remote-drive__auto-card">
        <span class="remote-drive__auto-label">清扫进度</span>
    ...
    <template v-else>
      ...
      <div class="remote-drive__bottom">
        <div class="remote-drive__tank">
          <div class="remote-drive__tank-row">
            <span>水箱</span>
            <div class="remote-drive__bar"><div class="remote-drive__bar-fill" :style="{ width: waterLevel + '%' }" /></div>
            <span>{{ waterLevel }}%</span>
          </div>
          <div class="remote-drive__tank-row">
            <span>垃圾箱</span>
            <div class="remote-drive__bar"><div class="remote-drive__bar-fill remote-drive__bar-fill--warn" :style="{ width: wasteLevel + '%' }" /></div>
            <span>{{ wasteLevel }}%</span>
          </div>
        </div>
        <div class="remote-drive__devices">
          <span v-for="d in deviceStatus" :key="d.key" class="remote-drive__device" :class="{ 'remote-drive__device--on': d.on }">
            {{ d.label }}
          </span>
        </div>
        <div class="remote-drive__progress">
          <span>清扫 {{ cleanProgress }}%</span>
          <div class="remote-drive__progress-ring">{{ cleanProgress }}</div>
        </div>
        <div class="remote-drive__actions">
          ...
          <button
            ...
            :class="{ 'remote-drive__action-btn--takeover-on': takeover }"
...
const deviceStatus = [
  { key: 'left-brush', label: '左盘刷', on: true },
  { key: 'right-brush', label: '右盘刷', on: true },
  { key: 'main-brush', label: '主刷', on: true },
  { key: 'nozzle', label: '吸嘴', on: false }
]

const gears = ['P', 'N', 'R', 'D']
const auxOn = ref(new Set<string>(['clean', 'high-beam']))
...
const modeStatusText = computed(() => {
  ...
  return takeover.value ? '远程接管中' : '监控模式'
})
