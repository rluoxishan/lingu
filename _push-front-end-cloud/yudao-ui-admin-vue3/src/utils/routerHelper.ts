...
const VIEW_COMPONENT_FALLBACK: Record<string, () => Promise<unknown>> = {
  'car/car': () => import('@/views/car/list/index.vue'),
  'car/car/index': () => import('@/views/car/list/index.vue'),
  'car/list': () => import('@/views/car/list/index.vue'),
  'car/list/index': () => import('@/views/car/list/index.vue'),
  'car/task': () => import('@/views/car/task/index.vue'),
  'car/task/index': () => import('@/views/car/task/index.vue'),
...
const findViewModule = (component?: string, path?: string, fullPath?: string) => {
  ...
  // 兜底：仅匹配明确的车辆/任务列表路径（避免 car/task 误落到车辆页）
  const normalizedFullPath = fullPath ? sanitizeViewKeyword(fullPath) : ''
  if (normalizedFullPath === 'car/car' || normalizedFullPath === 'car/car/index') {
    return VIEW_COMPONENT_FALLBACK['car/list']
  }
  if (normalizedFullPath === 'car/list' || normalizedFullPath === 'car/list/index') {
    return VIEW_COMPONENT_FALLBACK['car/list']
  }
  if (normalizedFullPath === 'car/task' || normalizedFullPath === 'car/task/index') {
    return VIEW_COMPONENT_FALLBACK['car/task']
  }
