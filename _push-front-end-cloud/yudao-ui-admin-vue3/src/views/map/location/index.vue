  ...
  points.forEach((item) => {
    ...
    if (existingMarker) {
      ...
      if (distance > 0.1) {
        ...
        console.log(`车辆 ${item.name} 位置变化: ${distance.toFixed(2)}米, 动画时长: ${duration}ms`)

        // 使用 moveTo 实现平滑移动动画
        existingMarker.moveTo(newPos, {
          duration: duration, // 动态动画时长
          delay: 0,
          autoRotation: false
        })
      } else if (distance > 0) {
...
const handleCarInfo = (data: any[], regionLnglat: string | string[], isPolling = false) => {
  ...
  data.forEach((item) => {
    ...
    // 轮询时记录位置变化
    if (isPolling && lnglat) {
      console.log(`车辆 ${item.name} 当前位置: [${lnglat[0]}, ${lnglat[1]}]`)
    }
...
const startPolling = () => {
  ...
  if (!currentRegion) {
    console.warn('没有选中的区域，无法启动轮询')
    return
  }
  console.log('启动车辆定位轮询，间隔10秒')
  pollingTimer = setInterval(() => {
    if (currentRegion) {
      console.log('轮询获取车辆定位...')
...
const handlePathData = (pathData, regionName) => {
  ...
  console.log(`路径数据已缓存，区域: ${regionName}，包含 ${processedPaths.length} 条路径段`)
...
const getRegionInfo = async (e) => {
  ...
  if (pathDataCache.has(regionName)) {
    ...
    console.log('使用缓存的路径数据:', regionName)
    showRegionPaths(cachedData.processedPaths)
    return
  }

  try {
    let data = await LocationApi.getRegionPath(regionName)
    console.log('获取到新的路径数据:', data)
...
const showRegionPaths = (processedPaths) => {
  ...
  console.log(`已显示 ${processedPaths.length} 条路径段，${siteMarkers.length} 个站点标记`)
}
...
const createSiteMarker = (position: number[], siteName: string, color: string, type: 'start' | 'end'): any => {
  ...
  // 添加点击事件
  marker.on('click', () => {
    console.log('点击了站点标记:', siteName, type)
