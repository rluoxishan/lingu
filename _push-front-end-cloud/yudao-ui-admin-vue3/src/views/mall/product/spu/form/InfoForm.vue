...
<template>
  <el-form ref="formRef" :disabled="isDetail" :model="formData" :rules="rules" label-width="120px">
    <el-form-item label="店铺名称" prop="storeId">
      <el-select v-model="formData.storeId" class="w-80" placeholder="请选择店铺" @change="handleStore">
        <el-option v-for="item in shopList" :key="item.id" :label="item.name" :value="item.id" />
      </el-select>
    </el-form-item>
    <el-form-item label="商品名称" prop="name">
      <el-input v-model="formData.name" :autosize="{ minRows: 2, maxRows: 2 }" :clearable="true" :show-word-limit="true"
        class="w-80!" maxlength="64" placeholder="请输入商品名称" type="textarea" />
    </el-form-item>
    ...
    <el-form-item label="商品简介" prop="introduction">
      <el-input v-model="formData.introduction" :autosize="{ minRows: 2, maxRows: 2 }" :clearable="true"
        :show-word-limit="true" class="w-80!" maxlength="128" placeholder="请输入商品简介" type="textarea" />
    </el-form-item>
...
const shopList = ref<ShopApi.ShopVO[]>([]) // 所有店铺
onMounted(async () => {
  // 获得分类树
...
// 处理店铺名称
const handleStore = (e) => {
  formData.storeName = shopList.value.filter(f => f.id === e)[0].name
}
</script>
