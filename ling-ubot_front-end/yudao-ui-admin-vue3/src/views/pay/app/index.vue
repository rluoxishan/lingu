  ...
  <ContentWrap>
    <el-form ref="queryFormRef" :inline="true" :model="queryParams" class="-mb-15px" label-width="68px">
      <el-form-item label="应用名" prop="name">
        <el-input
v-model="queryParams.name" class="!w-240px" clearable placeholder="请输入应用名"
...
v-model="scope.row.status" :active-value="0" :inactive-value="1"
      ...
      <el-table-column align="center" label="支付宝配置">
        <el-table-column
v-for="channel in alipayChannels" :key="channel.code" :label="channel.name.replace('支付宝', '')"
          align="center">
          <template #default="scope">
            <el-button
v-if="isChannelExists(scope.row.channelCodes, channel.code)" circle size="small" type="success"
      ...
      <el-table-column align="center" label="微信配置">
        <el-table-column
v-for="channel in wxChannels" :key="channel.code" :label="channel.name.replace('微信', '')"
          align="center">
          <template #default="scope">
            <el-button
v-if="isChannelExists(scope.row.channelCodes, channel.code)" circle size="small" type="success"
      ...
      <el-table-column align="center" label="钱包支付配置">
        <el-table-column :label="PayChannelEnum.WALLET.name" align="center">
          <template #default="scope">
            <el-button
v-if="isChannelExists(scope.row.channelCodes, PayChannelEnum.WALLET.code)" circle size="small"
              type="success" @click="openChannelForm(scope.row, PayChannelEnum.WALLET.code)">
              <Icon icon="ep:check" />
            </el-button>
            <el-button
v-else circle size="small" type="danger"
      ...
      <el-table-column align="center" label="模拟支付配置">
        <el-table-column :label="PayChannelEnum.MOCK.name" align="center">
          <template #default="scope">
            <el-button
v-if="isChannelExists(scope.row.channelCodes, PayChannelEnum.MOCK.code)" circle size="small"
...
v-else circle size="small" type="danger"
      ...
      <el-table-column align="center" label="第三方支付配置">
        <el-table-column :label="PayChannelEnum.THIRD.name" align="center">
          <template #default="scope">
            <el-button
v-if="isChannelExists(scope.row.channelCodes, PayChannelEnum.THIRD.code)" circle size="small"
              type="success" @click="openChannelForm(scope.row, PayChannelEnum.THIRD.code)">
              <Icon icon="ep:check" />
            </el-button>
            <el-button
v-else circle size="small" type="danger"
              @click="openChannelForm(scope.row, PayChannelEnum.THIRD.code)">
              <Icon icon="ep:close" />
            </el-button>
          </template>
        </el-table-column>
...
v-model:limit="queryParams.pageSize" v-model:page="queryParams.pageNo" :total="total"
  ...
  <ThirdChannelForm ref="thirdFormRef" @success="getList" />
</template>
...
import ThirdChannelForm from './components/channel/ThirdChannelForm.vue'
...
const openChannelForm = async (row, payCode) => {
  ...
  if (payCode.indexOf('school') === 0) {
    thirdFormRef.value.open(row.id, payCode)
  }
}
