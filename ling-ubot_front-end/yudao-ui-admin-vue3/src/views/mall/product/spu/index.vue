  ...
  <ContentWrap>
    <el-form ref="queryFormRef" :inline="true" :model="queryParams" class="-mb-15px" label-width="68px">
      <el-form-item label="店铺名称" prop="storeId">
        <el-select v-model="queryParams.storeId" class="!w-240px" placeholder="请选择车辆状态" @change="handleStoreChange">
      ...
      <el-form-item label="货道" prop="slotNumber">
        <el-input-number v-model.trim="queryParams.slotNumber" :min="0" :max="60" class="!w-240px" />
      </el-form-item>
      <el-form-item>
  ...
  <ContentWrap>
    ...
    <el-table v-loading="loading" :data="list">
                ...
                <el-row>
                  ...
                  <el-col :span="8">
                    <el-form-item label="货道:">
                      <span>{{ row.slotNumberSet || 'N/A' }}</span>
                    </el-form-item>
                  </el-col>
                </el-row>
              </el-col>
      ...
      <el-table-column align="center" fixed="right" label="操作" min-width="280">
        <template #default="{ row }">
          <el-button link type="primary" @click="openForm(row.id, 'copy')"> 克隆 </el-button>
...
const getTabsCount = async () => {
  const res = await ProductSpuApi.getTabsCount({ storeId: queryParams.value.storeId })
...
const resetQuery = () => {
  ...
  queryParams.value.storeId = shopList.value[0].id
  handleQuery()
}

/** 新增或修改 */
const openForm = (id?: number, formType?: string) => {
  // 修改
  if (typeof id === 'number') {
    if (formType === 'copy') {
      push({ name: 'ProductSpuCopy', params: { id } })
    } else {
...
onMounted(async () => {
  ...
  shopList.value = await ShopApi.getAllShop()
