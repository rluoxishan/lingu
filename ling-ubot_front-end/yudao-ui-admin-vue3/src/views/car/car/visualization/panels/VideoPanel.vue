  ...
  <div class="video-panel" :class="`video-panel--mode-${controlMode}`">
    <div class="video-panel__toolbar">
      <div class="video-panel__mode-hint">
        <span class="video-panel__mode-dot"></span>
        {{ modeHint }}
      </div>
    ...
    <div class="video-panel__grid">
      ...
      >
        <div class="video-panel__cell-scan"></div>
        <div class="video-panel__cell-bg"></div>
        <video
          v-show="hasStream(slot.key)"
          :ref="(el) => setVideoRef(slot.key, el)"
          class="video-panel__video"
          autoplay
          playsinline
          muted
        ></video>
        <div v-show="!hasStream(slot.key)" class="video-panel__cell-content">
          <Icon
        ...
        <template v-if="slot.isMain">
          <div class="video-panel__hud-corner video-panel__hud-corner--tl"></div>
          <div class="video-panel__hud-corner video-panel__hud-corner--tr"></div>
          <div class="video-panel__hud-corner video-panel__hud-corner--bl"></div>
          <div class="video-panel__hud-corner video-panel__hud-corner--br"></div>
        </template>
        <span
...
const props = withDefaults(
  defineProps<{
    ...
    mainCam?: CameraKey
    streamMap?: Map<CameraKey, CameraStreamItem>
    onAttachStream?: (key: CameraKey, el: HTMLVideoElement | null) => void
  }>(),
...
const hasStream = (key: CameraKey) => Boolean(props.streamMap?.get(key)?.url)

const videoRefs = new Map<CameraKey, HTMLVideoElement>()

const setVideoRef = (key: CameraKey, el: Element | ComponentPublicInstance | null) => {
  if (el instanceof HTMLVideoElement) {
    videoRefs.set(key, el)
    if (hasStream(key)) props.onAttachStream?.(key, el)
  } else {
    videoRefs.delete(key)
    props.onAttachStream?.(key, null)
  }
}
