...
/** 列表/监控：是否有车辆定位（detail 或 details 里 position / position_xyz） */
export function vehicleHasMapPosition(row: VehicleStatusVO): boolean {
  return !!(row.positionXyz || row.positionLonLat || row.positionRaw)
}

/** 列表/监控：是否可拉区域任务路线（需 regionName + device_path_info） */
export function vehicleHasMapRoute(row: VehicleStatusVO): boolean {
  return !!row.regionName?.trim()
}

/** 地图能力摘要（列表展示用） */
export function formatMapCapability(row: VehicleStatusVO): string {
  const pos = vehicleHasMapPosition(row)
  const route = vehicleHasMapRoute(row)
  if (pos && route) return '定位+路线'
  if (pos) return '仅定位'
  if (route) return '仅路线'
  return '无'
}
